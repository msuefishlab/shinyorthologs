comparisonsUI <- function(id) {
    ...
    tagList(
        ...
        h2('Heatmaps'),
        p(
            'Note: the species where it does not have an ortholog identified are given a value of 0, which may bias the heatmap. Therefore, use complete ortholog groups'
        ),
...
comparisonsServer <- function(input, output, session) {
    output$heatmap = d3heatmap::renderD3heatmap({
        ...
        rs <-
            dbSendQuery(
                conn,
                "SELECT $$SELECT * FROM crosstab('SELECT ortholog_id, species_id, gene_id FROM orthologs WHERE ortholog_id IN %s ORDER  BY 1, 2') AS ct (ortholog_id varchar(255), $$ || string_agg(quote_ident(species_id), ' varchar(255), ' ORDER BY species_id) || ' varchar(255))' FROM species"
            )
